...
${format_default(timeout, "genitive")}`, { height: 250 });
  ...
  var Backpack = class extends Screen {
    ...
    async update() {
      ...
      App_default2.server.send("bpg", {
        [PacketDataKeys_default.USER_OBJECT_ID]: App_default2.user.objectId,
        [PacketDataKeys_default.TOKEN]: App_default2.user.token
      });
      const d = await App_default2.server.awaitPacket("bpg");
      const data = d.bp;
          ...
          if (itmt == 0) {
            ...
            if (bait.iea > 0) {
              ...
              select.onchange = async () => {
                App_default2.server.send("baied", {
                  bied: {
                    bio: id,
                    itmps: {
                ...
                await App_default2.server.awaitPacket("baiedd");
              ...
              App_default2.server.send("bairm", {
                birm: {
                  bio: id
                }
              });
              await App_default2.server.awaitPacket("bairmd");
              this.update();
            }
      ...
      if (bits) {
        for (let i = 0; i < bits.length; i++) {
          ...
          button.onclick = async () => {
            App_default2.server.send("bia", { bio });
            await App_default2.server.awaitPacket("biad");
          };
        }
      }
...
`, "<br/>");
      ...
      btnBackpack.onclick = () => App_default2.screen = new Backpack();
