...
var require_js_md5 = __commonJS((exports, module) => {
  (function() {
...
class OPFS {
  ...
  async writeFile(path, data) {
    ...
    if (data instanceof global.Uint8Array)
      await writable.write(new global.Blob([data]));
    else
  ...
  async rename(oldPath, newPath) {
    ...
    try {
      ...
      if (success)
        ...
        this.log(`Failed to delete original file during rename`);
      return success;
    } catch {
...
class InMemoryFS {
  ...
  async deleteDirectory(path, recursive = false) {
    ...
    const keysToDelete = Object.keys(this.data).filter((key) => key == path || key.startsWith(path + "/"));
  ...
  async rename(oldPath, newPath) {
    ...
    if (isDir) {
      const entries = Object.keys(this.data).filter((k) => k.startsWith(oldPath + "/"));
  ...
  async changeBackend(type = "InMemory", root, force = false) {
    ...
    if (t == "opfs") {
      if (location.protocol == "file:")
        throw new Error(`OPFS doesn't work on protocol file://`);
...
function getZoom() {
  ...
  if (transform && transform != "none") {
    ...
    if (match)
      return global2.parseFloat(match[1]);
  }
...
async function decompress(data) {
  ...
  if (!isGzip)
    throw error2("Data are not compressed gzip");
  try {
...
function drag(win, event) {
  ...
  function moveHandler(e) {
    ...
    let { offsetLeft: ox, offsetTop: oy, offsetHeight: oh, offsetWidth: ow } = el, or = ox + ow, ob = oy + oh;
...
function resizableDrag(win, event, direction) {
  ...
  const zoom = getZoom();
  const startX = event.clientX / zoom, startY = event.clientY / zoom;
  const { clientWidth: startWidth, clientHeight: startHeight } = el;
  ...
  function moveHandler(e) {
    ...
    if (newWidth > win.minWidth && direction.includes("w"))
      win.x = newLeft;
    if (newHeight > win.minHeight && direction.includes("n"))
      win.y = newTop;
  }
  function upHandler(e) {
    document.removeEventListener("mousemove", moveHandler, true);
...
class Window extends Events {
  ...
  #init() {
    ...
    for (const corner of corners) {
      ...
      for (const s in corner.style)
        handle.style[s] = corner.style[s];
    ...
    for (const edge of edges) {
      const handle = document.createElement("div");
      for (const s in edge.style)
        handle.style[s] = edge.style[s];
  ...
  resize(event, direction) {
    if (this.isMaximum || !this.resizable)
      return;
...
var POW_2_24 = 0.00000005960464477539063;
...
// core/src/image.ts
async function writeToFS(toPath = "/", structure, rewrite = false, start = () => {}, process = () => {}) {
...
var import_js_md5 = __toESM(require_js_md5(), 1);
function md5salt(string, salt = "azxsw", iterations = 5) {
  let result = string;
  for (let i = 0;i < iterations; i++) {
    result = import_js_md5.default(result + salt);
  }
  return result;
}
...
class Launcher {
  ...
  async readData() {
    if (!await fs_default.existsFile("/urlsVersions.json"))
      fs_default.writeFile(`/urlsVersions.json`, JSON.stringify(["./images/vanilla.js", "./vanilla.js"]));
    if (!await fs_default.existsFile("/versions.json"))
      fs_default.writeFile(`/versions.json`, "[]");
    if (!await fs_default.existsFile("/profiles.json"))
      fs_default.writeFile(`/profiles.json`, "[]");
    if (!await fs_default.existsFile("/options.json"))
      fs_default.writeFile(`/options.json`, JSON.stringify({
        version: "",
        profile: "",
        theme: "macos"
      }));
    ...
    div.appendChild(this.progressBar);
    {
      ...
      addTab("Профили", createElement("div", {}, (elem) => {
        ...
        this.listProfiles.onchange = (e) => {
          const p = this.profiles.find((e2) => e2.name == this.listProfiles.value || this.listProfiles.value == `Новый аккаунт (${e2.email})`);
          if (p) {
            const isNew = this.listProfiles.value == `Новый аккаунт (${p.email})`;
            if (isNew)
              selectedProfile.textContent = `Выбран: ` + noXSS(`Новый: ${p.email}`);
            else
      ...
      addTab("Версии", createElement("div", {}, (elem) => {
        ...
        function update() {
          ...
          for (const ver of self2.versions) {
            ...
            if (ver.scriptPath && checkVersions)
